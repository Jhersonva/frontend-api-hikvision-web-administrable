  ...
  <div class="p-6">
    <div class="max-w-5xl mx-auto bg-white rounded-2xl shadow-md p-6 border">
      ...
      <h2 class="text-2xl font-bold text-gray-800 mb-4">
        Instalación del Producto: {{ productName }}
      </h2>

      <!-- Formulario Installation -->
      <form @submit.prevent="saveInstallation" class="space-y-4">
        <div>
          <label class="block text-sm font-medium">Descripción</label>
          <textarea
            v-model="form.description_product_installation"
            rows="3"
...
import {
  ...
  updateWhatInclude,
  deleteWhatInclude
} from "../api/productInstallationService";
...
export default {
  ...
  try {
    const res = await getProductInstallation(this.productId);
    if (res.data && res.data.id) {
      this.form.id = res.data.id;
      this.form.description_product_installation = res.data.description_product_installation || "";
      this.previewImage = res.data.img_product_installation || null;
    ...
    try {
        let res;
        if (!this.form.id) {
        res = await createProductInstallation(this.productId, this.form);
        } else {
        res = await updateProductInstallation(this.productId, this.form);
        }

        this.message = "Instalación guardada correctamente";
        this.form.id = res.data.data.id;
        this.previewImage = res.data.data.img_product_installation_url || this.previewImage;
        this.form.description_product_installation = res.data.data.description_product_installation || "";
    } catch (error) {
        console.error(error);
        this.message = "Error al guardar instalación";
    }
    },

    onImageChange(e) {
      const file = e.target.files[0];
      this.form.img_product_installation = file;
